...
const translations = {
  PT: {
    ...
    intro: (
      <>
        {c("oklch")} é um modelo de cor mais recente projetado para ser
        perceptualmente uniforme, ou seja, a forma como as cores parecem aos
        nossos olhos corresponde à matemática por trás delas. Diferente de
        modelos mais antigos como {c("rgb")} ou {c("hsl")}, o OKLCH produz
        resultados previsíveis ao ajustar luminosidade, croma ou matiz.
      </>
    ),
    colorModels: "Modelos de cor",
    colorModelsP1: (
      <>
        A maioria dos desenvolvedores conhece {c("rgb")} e {c("hsl")}. Esses
        modelos são baseados em como as telas emitem luz, não em como percebemos
        cor. Essa desconexão faz com que duas cores com o mesmo valor de
        &quot;luminosidade&quot; no HSL possam parecer muito diferentes aos
        nossos olhos.
      </>
    ),
    colorModelsP2: (
      <>
        O OKLCH resolve isso. Ele é construído sobre o espaço de cor OKLab, um
        modelo matematicamente rigoroso que mapeia diretamente a percepção
        humana de cor. O &quot;OK&quot; representa uma versão aprimorada do
        modelo Lab original.
      </>
    ),
    structure: "Estrutura",
    ...
    consistentP1: (
      <>
        A maior vantagem do OKLCH: cores com o mesmo valor {c("L")} realmente
        parecem igualmente brilhantes. No HSL, um amarelo e um azul com a mesma
        luminosidade parecem completamente diferentes.
      </>
    ),
    consistentP2: (
      <>
        Abaixo, todo círculo tem {c("L = 0.7")} e {c("C = 0.15")}, com a matiz
        rotacionando de 0° a 330°. Note como todos compartilham o mesmo brilho
        percebido:
      </>
    ),
    hslCompare: (
      <>
        Agora compare com HSL: mesmo valor de &quot;luminosidade&quot; de 50%,
        mas brilho percebido totalmente diferente:
      </>
    ),
    ...
    browserP2: (
      <>
        O navegador ignora a segunda declaração se não entender {c("oklch")},
        usando o valor hexadecimal como fallback.
      </>
    ),
    more: "Mais",
    moreP: (
      <>
        O OKLCH já é usado nos design tokens deste site. Para um mergulho mais
        profundo e ferramentas interativas, veja
      </>
    ),
  },
  EN: {
    ...
    intro: (
      <>
        {c("oklch")} is a newer color model designed to be perceptually uniform,
        meaning the way colors look to our eyes matches the math behind them.
        Unlike older models like {c("rgb")} or {c("hsl")}, OKLCH produces
        predictable results when you adjust lightness, chroma, or hue.
      </>
    ),
    colorModels: "Color models",
    colorModelsP1: (
      <>
        Most developers are familiar with {c("rgb")} and {c("hsl")}. These
        models are based on how screens emit light, not on how we perceive
        color. That disconnect means two colors with the same
        &quot;lightness&quot; value in HSL can look very different to our eyes.
      </>
    ),
    colorModelsP2: (
      <>
        OKLCH solves this. It&apos;s built on the OKLab color space, a
        mathematically rigorous model that maps directly to human color
        perception. The &quot;OK&quot; stands for an improved version of the
        original Lab model.
      </>
    ),
    structure: "Structure",
    ...
    consistentP1: (
      <>
        The biggest advantage of OKLCH: colors with the same {c("L")} value
        actually look equally bright. In HSL, a yellow and a blue at the same
        lightness look completely different.
      </>
    ),
    consistentP2: (
      <>
        Below, every circle has {c("L = 0.7")} and {c("C = 0.15")}, with hue
        rotating from 0° to 330°. Notice how they all share the same perceived
        brightness:
      </>
    ),
    hslCompare: (
      <>
        Now compare with HSL: same &quot;lightness&quot; value of 50%, but
    ...
    browserP2: (
      <>
        The browser ignores the second declaration if it doesn&apos;t understand{" "}
        {c("oklch")}, falling back to the hex value.
      </>
    ),
    more: "More",
    moreP: (
      <>
        OKLCH is already used in this site&apos;s own design tokens. For a
        deeper dive and interactive tools, check
      </>
    ),
  },
  ES: {
    ...
    intro: (
      <>
        {c("oklch")} es un modelo de color más reciente diseñado para ser
        perceptualmente uniforme, es decir, la forma en que los colores se ven a
        nuestros ojos coincide con la matemática detrás de ellos. A diferencia
        de modelos más antiguos como {c("rgb")} o {c("hsl")}, OKLCH produce
        resultados predecibles al ajustar luminosidad, croma o tono.
      </>
    ),
    colorModels: "Modelos de color",
    colorModelsP1: (
      <>
        La mayoría de los desarrolladores conocen {c("rgb")} y {c("hsl")}. Estos
        modelos se basan en cómo las pantallas emiten luz, no en cómo percibimos
        el color. Esa desconexión significa que dos colores con el mismo valor
        de &quot;luminosidad&quot; en HSL pueden verse muy diferentes a nuestros
        ojos.
      </>
    ),
    colorModelsP2: (
      <>
        OKLCH resuelve esto. Está construido sobre el espacio de color OKLab, un
        modelo matemáticamente riguroso que mapea directamente la percepción
        humana del color. El &quot;OK&quot; representa una versión mejorada del
        modelo Lab original.
      </>
    ),
    ...
    consistentP1: (
      <>
        La mayor ventaja de OKLCH: colores con el mismo valor {c("L")} realmente
        se ven igualmente brillantes. En HSL, un amarillo y un azul con la misma
        luminosidad se ven completamente diferentes.
      </>
    ),
    consistentP2: (
      <>
        Abajo, cada círculo tiene {c("L = 0.7")} y {c("C = 0.15")}, con el tono
        rotando de 0° a 330°. Note cómo todos comparten el mismo brillo
        percibido:
      </>
    ),
    hslCompare: (
      <>
        Ahora compare con HSL: mismo valor de &quot;luminosidad&quot; de 50%,
        pero brillo percibido totalmente diferente:
      </>
    ),
    ...
    browserP2: (
      <>
        El navegador ignora la segunda declaración si no entiende {c("oklch")},
        usando el valor hexadecimal como fallback.
      </>
    ),
    more: "Más",
    moreP: (
      <>
        OKLCH ya se usa en los design tokens de este sitio. Para una inmersión
        más profunda y herramientas interactivas, consulte
      </>
    ),
  },
};
    ...
    <div className="relative flex min-h-[100dvh] w-full flex-col overflow-x-hidden">
      ...
      <main className="mx-auto w-full max-w-(--breakpoint-sm) flex-1 px-4 py-12 leading-relaxed sm:py-20">
        ...
        <article>
          ...
          <SectionHeading id="consistent-brightness">
            {t.consistentBrightness}
          </SectionHeading>
            ...
            <div className="preview-card w-full p-1">
              <div className="flex h-full w-full flex-col items-center justify-center gap-1 sm:flex-row">
                {[
                  {
                    label: "sRGB",
                    gradient: "linear-gradient(in srgb, #ff00ff, #00ff00)",
                  },
                  {
                    label: "OKLAB",
                    gradient: "linear-gradient(in oklab, #ff00ff, #00ff00)",
                  },
                  {
                    label: "OKLCH",
                    gradient: "linear-gradient(in oklch, #ff00ff, #00ff00)",
                  },
                ].map(({ label, gradient }) => (
                  <div
                    key={label}
                  ...
                  >
                    <span className="px-3 py-2 font-mono text-xs text-muted-foreground">
                      {label}
                    </span>
                    <div
                      className="h-16 w-full rounded-b-lg sm:h-24"
          ...
          <SectionHeading id="browser-support">
            {t.browserSupport}
          </SectionHeading>
          ...
          <p className="mb-8 w-full text-pretty text-muted-foreground">
            {t.browserP2}
          </p>

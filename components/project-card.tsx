...
const DECK_LAYERS = 5
/** Sliver of each trailing card left visible, in px. */
const DECK_STEP_X = 18
/** Each trailing card starts slightly lower, in px. */
const DECK_STEP_Y = 5
/** Extra spread per trailing card on hover, in px. */
const DECK_FAN = 3
/** Hover lift per layer — nearest card rises most, so the fan arcs upward. */
const DECK_RISE = 4
/**
 * Trailing cards run well past the divider and get clipped there, so lifting
 * them on hover never exposes a gap above the dotted line.
 */
const DECK_TRAIL_BLEED = -40
      ...
      {Array.from({ length: DECK_LAYERS }, (_, index) => {
        ...
        return (
          <div
            key={layer}
            aria-hidden
            className="absolute w-(--deck-width) rounded-t-lg bg-preview-bg shadow-custom transition-transform duration-500 ease-[var(--ease-out-strong)] group-hover:translate-x-(--fan) group-hover:translate-y-(--rise) motion-reduce:transition-none"
            style={
              {
                left: layer * DECK_STEP_X,
                top: layer * DECK_STEP_Y,
                bottom: DECK_TRAIL_BLEED,
                "--deck-width": DECK_WIDTH,
                "--fan": `${layer * DECK_FAN}px`,
                "--rise": `-${(DECK_LAYERS - layer + 1) * DECK_RISE}px`,
...
export function ProjectCard({
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-dotted border-[var(--color-preview-border)]" />

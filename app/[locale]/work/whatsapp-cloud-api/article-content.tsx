...
const translations = {
  PT: {
    ...
    intro: (
      <>
        A Cloud API da Meta, a API oficial do WhatsApp, permite enviar botões,
        listas e carrosséis nativos sem BSP intermediário. Aqui vai o essencial
        para integrar e o padrão que mantém o estado da conversa fora do
        servidor.
      </>
    ),
    ...
    typesDesc: (
      <>
        Todos usam {c("POST /messages")} com{" "}
        {c('"messaging_product": "whatsapp"')}. A janela de 24h conta a partir
    ...
    typeRows: [
      {
        type: "text",
        label: "Texto simples",
        note: "Suporta *negrito*, _itálico_, `código`",
        window: "Qualquer hora",
      },
      {
        type: "button",
        label: "Botões",
        note: "Máx 3 botões, title até 20 chars",
        window: "24h",
      },
      {
        type: "list",
        label: "Lista",
        note: "4-10 opções em seções com modal",
        window: "24h",
      },
      {
        type: "carousel",
        label: "Carrossel",
        note: "2-10 cards com imagem + botão",
        window: "24h, ≥ 2 cards",
      },
    ],
    ...
    buttonsDesc: (
      <>
        O campo {c("reply.id")} é retornado exato no webhook, use-o para
        identificar a ação sem estado no servidor.
      </>
    ),
    ...
    idsDesc: (
      <>
        A forma mais limpa de gerenciar fluxo de conversa é codificar a ação
        direto no {c("id")} do botão com prefixos. No webhook, parse o prefixo e
        execute a ação: o botão carrega a intenção, o servidor não precisa
        guardar nada.
      </>
    ),
    ...
    checklistItems: [
      ...
      <>
        Responder ao webhook com HTTP 200 imediatamente, a Meta retenta se não
        receber em 20s
      </>,
      <>
        Deduplicar por {c("message.id")}, a Meta pode entregar a mesma mensagem
        mais de uma vez
      </>,
      <>
        Envolver sends interativos em {c("try/catch")} com fallback para texto
        simples
      </>,
      <>
        Carrossel: mínimo 2 cards, todos com o mesmo número de botões, imagens
        JPEG 1:1
      </>,
    ],
    ...
    downloadDesc: (
      <>
        O arquivo cobre todos os payloads com limites exatos, estrutura do
        webhook para cada tipo (incluindo a diferença do{" "}
        {c("carousel quick_reply")}), estratégia de fallback e regras de imagem
        para carrossel.
      </>
    ),
  ...
  EN: {
    ...
    intro: (
      <>
        Meta&apos;s official WhatsApp API lets you send native buttons, lists
        and carousels, with no middleware BSP required. Here&apos;s what you
        need to integrate and the ID pattern that keeps conversation state off
        the server.
      </>
    ),
    ...
    typesDesc: (
      <>
        All types use {c("POST /messages")} with{" "}
        {c('"messaging_product": "whatsapp"')}. The 24h window starts from the
        user&apos;s last message.
      </>
    ),
    typeRows: [
      {
        type: "text",
        label: "Plain text",
        note: "Supports *bold*, _italic_, `code`",
        window: "Any time",
      },
      {
        type: "button",
        label: "Buttons",
        note: "Max 3 buttons, title up to 20 chars",
        window: "24h",
      },
      {
        type: "list",
        label: "List",
        note: "4-10 options in sections with modal",
        window: "24h",
      },
      {
        type: "carousel",
        label: "Carousel",
        note: "2-10 cards with image + button",
        window: "24h, ≥ 2 cards",
      },
    ],
    ...
    buttonsDesc: (
      <>
        The {c("reply.id")} field is returned exactly as sent in the webhook,
        use it to identify the action without any server-side state.
      </>
    ),
    ...
    idsDesc: (
      <>
        The cleanest way to manage conversation flow is to encode the action
        directly in the button {c("id")} with prefixes. In the webhook, parse
        the prefix and execute the action: the button carries the intent, no
        server state needed.
      </>
    ),
    ...
    checklistItems: [
      ...
      <>
        Respond to the webhook with HTTP 200 immediately, Meta retries if it
        doesn&apos;t receive a response in 20s
      </>,
      <>
        Deduplicate by {c("message.id")}, Meta may deliver the same message more
        than once
      </>,
      <>Wrap interactive sends in {c("try/catch")} with plain text fallback</>,
      <>
        Carousel: minimum 2 cards, all with the same number of buttons, JPEG 1:1
        images
      </>,
    ],
    ...
    downloadDesc: (
      <>
        The file covers all payloads with exact character limits, webhook
        structure for each type (including the {c("carousel quick_reply")}{" "}
        difference), fallback strategy, and carousel image rules.
      </>
    ),
  ...
  ES: {
    ...
    intro: (
      <>
        La Cloud API de Meta, la API oficial de WhatsApp, permite enviar
        botones, listas y carruseles nativos sin BSP intermediario. Aquí va lo
        esencial para integrar y el patrón que mantiene el estado de
        conversación fuera del servidor.
      </>
    ),
    ...
    typesDesc: (
      <>
        Todos usan {c("POST /messages")} con{" "}
        {c('"messaging_product": "whatsapp"')}. La ventana de 24h comienza desde
    ...
    typeRows: [
      {
        type: "text",
        label: "Texto simple",
        note: "Soporta *negrita*, _cursiva_, `código`",
        window: "Cualquier hora",
      },
      {
        type: "button",
        label: "Botones",
        note: "Máx 3 botones, title hasta 20 chars",
        window: "24h",
      },
      {
        type: "list",
        label: "Lista",
        note: "4-10 opciones en secciones con modal",
        window: "24h",
      },
      {
        type: "carousel",
        label: "Carrusel",
        note: "2-10 cards con imagen + botón",
        window: "24h, ≥ 2 cards",
      },
    ],
    ...
    buttonsDesc: (
      <>
        El campo {c("reply.id")} se devuelve exacto en el webhook, úsalo para
        identificar la acción sin estado en el servidor.
      </>
    ),
    ...
    idsDesc: (
      <>
        La forma más limpia de gestionar el flujo de conversación es codificar
        la acción directamente en el {c("id")} del botón con prefijos. En el
        webhook, parsea el prefijo y ejecuta la acción: el botón lleva la
        intención, el servidor no necesita guardar nada.
      </>
    ),
    ...
    checklistItems: [
      ...
      <>
        Responder al webhook con HTTP 200 inmediatamente, Meta reintenta si no
        recibe respuesta en 20s
      </>,
      <>
        Deduplicar por {c("message.id")}, Meta puede entregar el mismo mensaje
        más de una vez
      </>,
      <>
        Envolver sends interactivos en {c("try/catch")} con fallback a texto
        simple
      </>,
      <>
        Carrusel: mínimo 2 cards, todos con el mismo número de botones, imágenes
        JPEG 1:1
      </>,
    ],
    ...
    downloadDesc: (
      <>
        El archivo cubre todos los payloads con límites exactos, estructura del
        webhook para cada tipo (incluida la diferencia del{" "}
        {c("carousel quick_reply")}), estrategia de fallback e imágenes para
        carrusel.
      </>
    ),

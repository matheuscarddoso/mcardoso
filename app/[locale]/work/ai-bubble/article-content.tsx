...
const translations = {
  PT: {
    ...
    intro: (
      <>
        Tenho 23 anos, estou na área de tecnologia há cinco, sou formado em
        Ciência da Computação e fiz meu TCC justamente sobre IA generativa. Será
        que sou só eu, ou essa bolha está ficando cada vez mais inflada? Essas
        empresas continuam queimando caminhões de dinheiro todo mês e, de
        repente, o tom dos executivos mudou.
      </>
    ),

    tone: "O tom mudou",
    toneP1:
      "Agora eles empurram IPOs com valuations maiores do que qualquer coisa já vista, e ao mesmo tempo gente como Sam Altman pede ajuda do governo para sustentar financeiramente as próprias empresas. Isso é comportamento clássico de bolha no auge, exatamente o tipo de coisa que se vê pouco antes de estourar. Todo mundo correndo atrás do próximo investidor disposto a ficar com o abacaxi antes que a música pare.",
    toneP2:
      "Mas por que agora? Houve uma série de mudanças recentes que expõem a fragilidade estrutural dessa bolha. Vale dar um passo atrás.",

    agi: "Não temos AGI",
    agiP1:
      "Sei que isso incomoda quem está convicto de que a AGI está logo ali. Mas não temos AGI. Surgiram loops de agentes bem elaborados e integrações de ferramentas boas, sim. No fundo, porém, o estado atual da IA ainda é um papagaio probabilístico prevendo a palavra mais provável a partir de dados de treinamento passados. Um modelo assim não é capaz de raciocínio ou lógica de verdade.",
    agiP2:
      "E por conta dessa natureza derivativa, a IA não é capaz de inovações originais que aumentem a produtividade econômica de forma fundamental. Ela não vai inventar o motor de dobra tão cedo. O que a tecnologia atual consegue fazer, de forma plausível, é substituir trabalho cognitivo humano repetitivo.",

    debt: "A conta da dívida",
    debtP1:
      "Minha convicção é que o boom da IA é uma aposta gigantesca e altamente alavancada em que a IA vai conseguir substituir esse trabalho humano de forma lucrativa. Por que a substituição de mão de obra é o uso mais lógico? Pelos números.",
    debtP2:
      "Entre três e quatro trilhões de dólares já foram investidos na indústria americana de IA. Parte vem de capital de investidores, mas a maior parte está na forma de dívida corporativa. E dívida precisa ser paga.",
    debtP3:
      "A uma taxa normal para títulos corporativos, entre 3% e 4%, dois a três trilhões em dívida geram cerca de cem bilhões de dólares em juros por ano. A indústria precisa gerar pelo menos isso em lucro só para empatar.",
    debtP4:
      "Suponha uma margem de 10% (que eles não têm, mas suponha). Para gerar esse lucro, a indústria precisaria substituir uma fatia da economia americana equivalente a mais ou menos um trilhão de dólares por ano. E a única fatia grande o suficiente é o mercado de trabalho de colarinho branco, avaliado em dez trilhões.",
    debtP5:
      "Um trilhão em empregos de colarinho branco é o mesmo que dizer dez milhões de trabalhadores americanos por ano. É esse plano que explica por que Sam Altman e Dario Amodei vêm profetizando há anos um apocalipse do emprego.",

    china: "Os modelos abertos chineses",
    chinaP1:
      "Só que o plano não está saindo como esperado. Modelos de fronteira americanos, como ChatGPT e Claude, são fechados: as empresas controlam algoritmos, dados e infraestrutura, e vendem acesso por assinatura. O problema é que treinar e operar esses modelos é absurdamente caro. A operação inteira é deficitária: elas perdem dinheiro em cada chamada de API.",
    chinaP2:
      "Isso, sozinho, não seria novidade. Empresas de tecnologia há muito subsidiam serviço abaixo do custo, ganham participação, viram monopólio e só então aumentam preços. O problema é que dessa vez os concorrentes não morreram.",
    chinaP3:
      "Com uma fração dos recursos computacionais dos Estados Unidos, empresas chinesas criaram modelos de código aberto competitivos: pela maioria das métricas um pouco atrás, empatados ou até à frente dos melhores modelos americanos. Sendo abertos, podem ser baixados de graça e executados na infraestrutura do próprio cliente, por uma fração minúscula do custo.",
    chinaP4:
      "Venho usando o Kimi3, da Moonshot, há alguns dias, e o desempenho é comparável ao da versão mais limitada do Claude Fable 5 para os meus casos de uso. E não sou o único a perceber: segundo a OpenRouter, modelos chineses de código aberto já representam mais de 60% de todos os tokens usados por empresas americanas.",
    chinaP5:
      "Ou seja: a ideia de criar um monopólio, aumentar os preços e faturar alto já não está na mesa.",

    local: "E os modelos locais",
    localP1:
      "Dá para pegar um modelo de fronteira aberto e, com quantização e destilação, comprimir esse modelo enorme em um modelo local muito menor. Em vez de um data center gigantesco, ele roda no seu computador de casa, num servidor doméstico ou até num notebook decente.",
    localP2:
      "São duas vantagens. Rodando localmente, você não paga assinatura nenhuma para as grandes empresas de tecnologia. E funcionam totalmente offline, o que garante privacidade sobre os próprios dados.",
    localP3:
      "Nos últimos meses esses modelos ficaram surpreendentemente capazes. Um bom modelo local, como o Qwen 3.5, é mais ou menos equivalente ao Claude Sonnet 4 na maioria das tarefas. À medida que tarefas básicas passam a ser resolvidas assim, a disposição de pagar por IA premium cai drasticamente.",

    productivity: "A produtividade não chegou",
    productivityP1:
      "E aqui está o motivo mais importante: os ganhos de produtividade estão acontecendo muito mais devagar do que o esperado. Não estamos vendo nem de longe dez milhões de trabalhadores de colarinho branco sendo demitidos por IA este ano.",
    productivityP2a:
      "Já escrevi sobre minha experiência com IA agêntica como engenheiro de software",
    productivityP2b:
      " e sobre o trabalho que dá extrair um resultado de qualidade: gerenciar contexto, contornar lacunas nos dados de treinamento, criar fluxos consistentes, evitar alucinações. Esses desafios são reais. Exige muito trabalho e muita inteligência humana usar IA de forma produtiva.",
    productivityP3:
  ...
  EN: {
    ...
    intro: (
      <>
        I&apos;m 23, five years into tech, with a computer science degree and a
        thesis on generative AI. Is it just me, or is this bubble getting more
        inflated by the day? These companies keep burning truckloads of money
        every month, and all of a sudden, the tone from the executives has
        changed.
      </>
    ),
    ...
    chinaP5:
      "So the idea that American AI companies could build a monopoly, raise prices and cash in is off the table.",
    ...
    productivityP2a:
      "I've written about my own experience with agentic AI as a software engineer",
    productivityP2b:
      " and about how much work it takes to get a quality result: managing context, working around gaps in the training data, building consistent workflows, avoiding hallucinations. Those challenges are real. Using AI productively takes a lot of effort and a lot of human intelligence.",
    productivityP3:
    ...
    outro:
      "That's what I had to say. If you'd like to keep the conversation going, find me on",
  },
  ...
  ES: {
    ...
    intro: (
      <>
        Tengo 23 años, llevo cinco en tecnología, soy graduado en Ciencias de la
        Computación e hice mi tesis justamente sobre IA generativa. ¿Seré solo
        yo, o esta burbuja está cada vez más inflada? Estas empresas siguen
        quemando camiones de dinero todos los meses y, de repente, el tono de
        los ejecutivos cambió.
      </>
    ),
    ...
    productivityP2a:
      "Ya escribí sobre mi propia experiencia con IA agéntica como ingeniero de software",
    productivityP2b:
      " y sobre el trabajo que cuesta obtener un resultado de calidad: gestionar contexto, sortear lagunas en los datos de entrenamiento, crear flujos consistentes, evitar alucinaciones. Esos desafíos son reales. Usar IA de forma productiva exige mucho trabajo y mucha inteligencia humana.",
    productivityP3:
    ...
    outro:
      "Eso es lo que tenía para decir. Si querés seguir la conversación, me encontrás en",
  },
};
        ...
        <article>
          ...
          <p className="mb-4 w-full text-pretty text-muted-foreground">
            <Link
              ...
              href={`/${locale}/work/saving-claude-tokens`}
            >
              {t.productivityP2a}
            </Link>
            {t.productivityP2b}

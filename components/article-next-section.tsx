...
export function ArticleNextSection({ language }: { language: Language }) {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          initial={{ opacity: 0, scale: 0.9, filter: "blur(4px)", y: 8 }}
          animate={{ opacity: 1, scale: 1, filter: "blur(0px)", y: 0 }}
          exit={{ opacity: 0, scale: 0.92, filter: "blur(4px)", y: 8 }}
        ...
        >
          <button
            ...
            /* Glass, and only just: the surface is mostly opaque so the arrow
               stays legible over whatever paragraph is behind it, with enough
               blur that the page reads as continuing underneath rather than
               stopping at a chip. */
            className="pointer-events-auto grid size-9 cursor-pointer place-items-center rounded-full bg-preview-bg/75 text-gray-1100 shadow-card-lift ring-1 ring-border backdrop-blur-md backdrop-saturate-150 transition-[color,transform] duration-150 ease-[var(--ease-out-strong)] hover:bg-preview-bg/90 hover:text-gray-1200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black/40 active:scale-[0.94] motion-reduce:active:scale-100 dark:focus-visible:outline-white/40"
          >
